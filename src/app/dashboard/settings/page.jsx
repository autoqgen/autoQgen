...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    const normalizedSubdomain = userName.toLowerCase().replace(/[^a-z0-9]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "");
  ...
  const handleSaveInfo = () => {
    setUserDetails((prev) => ({
      ...
      subdomain: tempInfo.name.toLowerCase().replace(/[^a-z0-9]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, ""),
    }));
      ...
      {activeTab === "account" ? (
              ...
              {!isEditingInfo ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-5 gap-x-8">
                  ...
                  <div>
                    ...
                    <div className="font-semibold text-gray-800">{userDetails.email}</div>
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-gray-400 block mb-1">PHONE NUMBER</label>
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-gray-800">{userDetails.phone}</span>
                      <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-bold text-gray-500 block mb-1.5">Name</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="text-xs font-bold text-gray-500 block mb-1.5">Role</label>
                    <input
                      type="text"
              ...
              {!isChangingPassword ? (
                ...
                <form onSubmit={handleSavePassword} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                      <label className="text-xs font-bold text-gray-500 block mb-1.5">Confirm Password</label>
                      <input
                        type="password"
                        placeholder="••••••••"
                        className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:border-purple-500 text-gray-800 text-sm"
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm transition-all hover:shadow-md">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-xs font-semibold text-gray-400 block mb-1">NAME</label>
                  <div className="font-bold text-gray-800 text-sm sm:text-base truncate">{userDetails.orgName}</div>
                </div>

                <div>
                  <label className="text-xs font-semibold text-gray-400 block mb-1">SUBDOMAIN</label>
                ...
                <div>
                  <label className="text-xs font-semibold text-gray-400 block mb-1">UUID</label>
                  <div className="flex items-center justify-between bg-gray-50 border border-gray-100 p-2.5 rounded-xl gap-2 max-w-full">
                    ...
                    <button
                      ...
                      title="Copy UUID"
                    >
                      {copiedField === "uuid" ? <Check size={14} className="text-emerald-500" /> : <Copy size={14} />}
                    </button>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm transition-all hover:shadow-md">
              ...
              <div className="bg-[#FAF5FF] border border-purple-100/50 rounded-2xl p-5 space-y-4">
                <div className="space-y-1">
                  <h3 className="font-bold text-gray-800 text-sm">Log Out From All Devices</h3>
                  <p className="text-xs font-medium text-gray-500 leading-relaxed">
                    Log out from all other active sessions and devices for enhanced security.
                  </p>
                </div>
        ...
        <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-gray-50 pb-5">
            <div>
              <h2 className="font-extrabold text-gray-800 text-xl">Class Directory</h2>
              <p className="text-sm text-gray-500 mt-1">Manage class sections, view codes, and track total students.</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {classes.map((cls) => (
              ...
              >
                <div>
                  <h3 className="font-bold text-gray-800 text-lg leading-snug">{cls.name}</h3>
                ...
                <div className="flex items-center justify-between border-t border-gray-200/50 pt-3">
                  ...
                  <span className="text-sm font-bold text-gray-700">{cls.students} enrolled</span>
                </div>
              </div>
            ))}
          </div>
